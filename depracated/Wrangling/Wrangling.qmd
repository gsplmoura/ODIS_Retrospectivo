...
# another for overage consultations, and record_ids 
# who continued followup after coming of age.

consultas_underage <- obese %>% 
  arrange(desc(age)
    ) %>% 
    filter(age < 18
    ) %>% 
...
`overage` é o dataset de consultas contendo os registros dos pacientes maiores de idade que tiveram pelo menos um IMC > 30 e acompanhados durante o período de interesse, com um total de `r nrow(overage)` consultas médicas.
...
write_rds(overage, here("data_output","consultas_overage.rds"))

rm(list = setdiff(ls(), c("overage", "codebook_obese")))
```
